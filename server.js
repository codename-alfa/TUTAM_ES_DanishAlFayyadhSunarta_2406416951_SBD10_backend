...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Username sudah digunakan' });
    }
});
...
cron.schedule('1 0 * * *', async () => {
    try {
        ...
        for (let schedule of dueSchedules.rows) {
            await pool.query(
                'INSERT INTO transactions (title, amount, type, user_id) VALUES ($1, $2, $3, $4)',
                [schedule.title, schedule.amount, schedule.type, schedule.user_id]
            );

            let nextRunDate = new Date(schedule.next_run);
            if (schedule.frequency === 'DAILY') nextRunDate.setDate(nextRunDate.getDate() + 1);
            else if (schedule.frequency === 'WEEKLY') nextRunDate.setDate(nextRunDate.getDate() + 7);
            else if (schedule.frequency === 'MONTHLY') nextRunDate.setMonth(nextRunDate.getMonth() + 1);

            await pool.query(
                'UPDATE recurring_schedules SET next_run = $1 WHERE id = $2',
